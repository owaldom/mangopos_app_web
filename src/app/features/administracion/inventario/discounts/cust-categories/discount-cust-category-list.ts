...
import { DiscountCustCategoryFormComponent } from './discount-cust-category-form';
import { ConfirmDialogComponent } from '../../../../../shared/components/confirm-dialog/confirm-dialog.component';

@Component({
  selector: 'app-discount-cust-category-list',
  standalone: true,
  imports: [
    CommonModule,
    MatTableModule,
  ...
  template: `
    <div class="container">
      <div class="header">
        <h1>Categorías de Descuento (Clientes/Prov)</h1>
...
export class DiscountCustCategoryListComponent implements OnInit {
  categories: any[] = [];
  ...
  getCategories(): void {
    this.discountService.getDiscountCustCategories().subscribe((data: any[]) => {
      this.categories = data;
    });
  }

  openForm(category?: any): void {
    const dialogRef = this.dialog.open(DiscountCustCategoryFormComponent, {
      width: '400px',
      data: { category }
    });
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        if (category) {
          this.discountService.updateDiscountCustCategory(category.id, result).subscribe(() => {
            this.snackBar.open('Categoría actualizada', 'Cerrar', { duration: 3000 });
            this.getCategories();
          });
        } else {
          this.discountService.createDiscountCustCategory(result).subscribe(() => {
  ...
  deleteCategory(category: any): void {
    ...
    dialogRef.afterClosed().subscribe(confirmed => {
      if (confirmed) {
        this.discountService.deleteDiscountCustCategory(category.id).subscribe(() => {
